...
**Cross-platform status.** war-dogs is developed and driven on Linux. It reasons about other platforms at each site — the file opener picks `xdg-open`/`open`/`cmd start` and reports a missing one instead of crashing; `~/` expands through the home directory; the peer transport is a Unix socket on POSIX and a named pipe on Windows; background jobs are process-grouped on POSIX and killed by pid on Windows. **Windows is supported on Windows Terminal** (tested: Windows 11, Windows Terminal, PowerShell 5.1, Git for Windows, pi installed through `pi-node`). The Windows acceptance rig (`dev/instruments/win/`, a ConPTY driver) passes end to end: boot, off = stock (screen and palette byte-identical to `--no-extensions`), the switch round-trip, every exit path restoring the terminal, folds and the wheel by mouse, ctrl+g with a GUI editor, `powershell` as the shell in the session and in agents, background jobs, the station and run views, the default browser through `mcp` (Chrome found under Program Files, not on PATH), WebFetch's static and render tiers with the daemon, web search, image attachments with Windows paths (pi's paste-image key is `alt+v` there) including a dropped file with a space in its name, and a peer conversation between two sessions. Not covered: the classic conhost window (the mechanism underneath is the same ConPTY, the renderer is not), and `ctrl+z` (no SIGTSTP on Windows; nothing to suspend to). macOS is unverified.
