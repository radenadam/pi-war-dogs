...
war-dogs' own tools are `agent`, `webfetch`, `kimi-websearch` (`index.ts` `WAR_DOGS_TOOLS`), plus the re-skinned built-ins (`read`, `write`, `edit`, and the platform's shell: `bash`, or `powershell` on Windows), plus the MCP adapter's `mcp`, `mcpScript` and per-server tools. When war-dogs is off, none of these register: the model's tool list and system prompt are pi's, byte for byte.
